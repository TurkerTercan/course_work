public class RedBlackTree<E extends Comparable<E>> extends BinarySearchTreeWithRotate<E> {
    ...
    private Node<E> fixupRight(Node<E> localRoot)
    {
        ...
        { // Case 2, 3, or 4
            if ((s.left == null && s.right == null)
                    || ((s.left != null && !((RedBlackNode<E>) s.left).isRed) && (s.right != null && !((RedBlackNode<E>) s.right).isRed)))
            {
                // Case 2
                s.isRed = true;
                return localRoot;
    ...
    public String toString() {
        ...
        preorder((RedBlackNode)root, sb, 0);
        return sb.toString();
    }

    private void preorder( RedBlackNode<E> localRoot, StringBuilder sb, int n) {
        if (localRoot == null) {
            for (int i = 0; i < n; i++)
                sb.append("\t");
        ...
        sb.append(localRoot.toString()).append("\n");
        preorder((RedBlackNode<E>)localRoot.left, sb, n + 1);
        preorder((RedBlackNode<E>)localRoot.right, sb, n + 1);
    }
}